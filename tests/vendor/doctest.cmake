...
function(doctest_discover_tests TARGET)
  ...
  # ---------------------------------------------------------------------------
  # LOCAL PATCH (NexusBVH) -- multi-config correctness.
  #
  # Upstream names the generated files per target and per argument hash, but NOT per
  # configuration. On a multi-config generator (Visual Studio) the POST_BUILD step below
  # resolves $<TARGET_FILE:> to whichever config was just built and writes it into a
  # shared filename, so Debug and Release overwrite each other: after building Debug,
  # `ctest -C Release` silently launches the Debug binary.
  #
  # This module is a fork of an older CMake GoogleTest.cmake (see the CMake license
  # header at the top of this file). CMake fixed this in their copy by putting $<CONFIG>
  # into the generated filename and resolving it at ctest time through
  # CTEST_CONFIGURATION_TYPE; doctest's fork never picked the fix up, and it is still
  # absent from doctest master. This reproduces CMake's fix.
  # ---------------------------------------------------------------------------
  get_property(_doctest_is_multi_config GLOBAL PROPERTY GENERATOR_IS_MULTI_CONFIG)

  set(ctest_file_base "${CMAKE_CURRENT_BINARY_DIR}/${TARGET}")
  if(_doctest_is_multi_config)
    string(APPEND ctest_file_base "-$<CONFIG>")
  endif()

  # Define rule to generate test list for aforementioned test executable
  set(ctest_include_file "${ctest_file_base}_include-${args_hash}.cmake")
  set(ctest_tests_file "${ctest_file_base}_tests-${args_hash}.cmake")
  ...
  if(_doctest_is_multi_config)
    # The path itself carries $<CONFIG>, so it has to go through file(GENERATE),
    # which writes one file per configuration and resolves the genex in the name
    # and in the content.
    string(JOIN "" ctest_include_content ${ctest_include_content})
    file(GENERATE OUTPUT "${ctest_include_file}" CONTENT "${ctest_include_content}")

    # ctest cannot evaluate a genex, so point it at a config-less dispatcher that
    # resolves CTEST_CONFIGURATION_TYPE (set by `ctest -C <cfg>`) when the test list
    # is read, and includes the per-config file that matches.
    string(REPLACE "-$<CONFIG>" "-\${CTEST_CONFIGURATION_TYPE}"
      include_file_cfg "${ctest_include_file}")
    string(REPLACE "-$<CONFIG>" "" ctest_include_file "${ctest_include_file}")
    ...
    file(WRITE "${ctest_include_file}" ${ctest_include_content})
  endif()
